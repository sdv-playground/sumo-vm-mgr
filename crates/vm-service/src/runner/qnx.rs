...
///
/// Start flow:
///   1. devb-loopback maps rootfs file → /dev/qvmdiskN
///   2. qvm @<config> launches the guest VM
///   3. Track qvm PID for is_running/stop/wait
///
/// Cleanup kills both processes (qvm first, then loopback).
...
impl VmRunner for QnxRunner {
    fn start(&mut self, name: &str, def: &VmDefinition) -> Result<VmHandle, RunnerError> {
        ...
        // Syntax from QNX SDK: devb-loopback loopback prefix=qvmdisk,fd=<path>
